...
```bash
./setup-musicbrainz <replication-token> <data-dir>
```
...
```bash
docker compose up -d --no-build db valkey search musicbrainz indexer
...
`./setup-musicbrainz` includes `compose/live-indexing-search.yml` in the compose chain and configures SIR to poll PostgreSQL directly.

On a healthy setup it will:

- install the SIR schema and triggers if missing (`admin/setup-sir install`)
...
- start the `indexer` service (`python -m sir live`)
- keep the `indexer` service running in live mode afterward
...
```bash
docker compose stop musicbrainz indexer search valkey db
```
