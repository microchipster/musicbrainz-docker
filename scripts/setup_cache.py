...
def repair_solr_cache(args: argparse.Namespace) -> int:
    ...
    md5_by_name = {filename: checksum for checksum, filename in iter_md5_entries(md5_file, ".tar.zst")}
    ...
    for path in backup_dir.iterdir():
        ...
        if ".tmp." in path.name:
            ...
            final_path = path.with_name(path.name[: -len(".part")])
            checksum = md5_by_name.get(final_path.name)
            if final_path.is_file() and checksum and md5(final_path) == checksum:
                path.unlink(missing_ok=True)

    for filename, checksum in md5_by_name.items():
