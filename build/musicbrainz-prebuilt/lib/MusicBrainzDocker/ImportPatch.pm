...
sub _archive_member_for_table {
    ...
    return ('/media/dbdump/mbdump.tar.bz2', "mbdump/$table")
        if $table =~ /\A(?:isrc|iswc|l_artist_recording|l_artist_work|l_label_recording|l_recording_url|l_release_group_release_group|l_release_url|medium|recording|track|url)\z/;
...
sub _force_stream_for_table {
    ...
    return $table =~ /\A(?:cover_art_archive\.cover_art|isrc|iswc|l_artist_recording|l_artist_work|l_release_group_release_group|statistics\.statistic)\z/;
}

sub _trim_source_column_program {
    my ($archive, $member) = @_;

    return 'PROGRAM ' . _psql_literal(
        qq{tar -xOf $archive $member | awk -F '\\t' 'NF == 6 { printf "%s\\t%s\\t%s\\t%s\\t%s\\n", \$1, \$2, \$3, \$5, \$6; next } { print }'}
    );
}
...
sub _prepare_copy_source {
    ...
    if ($table =~ /\A(?:isrc|iswc)\z/ && $archive) {
        _debug("streaming trimmed schema 31-compatible source for table=$table from $archive via psql COPY PROGRAM");
        return (undef, _trim_source_column_program($archive, $member), undef);
    }
...
sub _copy_table_from_file_via_psql {
    ...
    my $pipe_closed = close $pipe;
    my $exit_code = (!$pipe_closed || $CHILD_ERROR != 0) ? 1 : 0;
