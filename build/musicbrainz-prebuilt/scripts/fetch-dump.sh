...
DB_DUMP_DIR="${MUSICBRAINZ_DB_DUMP_DIR:-/media/dbdump}"
SEARCH_DUMP_DIR="${MUSICBRAINZ_SEARCH_DUMP_DIR:-/media/searchdump}"
BASE_FTP_URL=''
...
then
    ...
    do
        echo -n "$F: "
        if printf '%s\n' "$(grep -F "*$F" MD5SUMS)" | md5sum -c --status
        then
            echo OK
        else
            echo FAILED
            echo >&2 "$0: unmatched MD5 checksum for $F"
            exit 70
        fi
    done
    cd - >/dev/null
...
then
    "${WGET_CMD[@]}" -c -P "$DB_DUMP_DIR" \
        "${BASE_DOWNLOAD_URL}/$DB_DUMP_REMOTE_DIR/$DB_DUMP_FILES"
fi
