...
* Current MB Branch: [v-2026-07-30.1](build/musicbrainz/Dockerfile#L98)
...
Long-lived mirrors accumulate failure modes that a fresh install never sees:
stalled live-indexing queues, stale replication windows, and Solr replicas
whose coordination state drifted until collections stop serving. `setup-musicbrainz`
installs a systemd user timer (`musicbrainz-selfheal.timer`, 30-minute cadence)
that runs `scripts/self-heal.sh`: it verifies the deployment and, only when
verification fails, applies escalating remedies. Every action is logged to
`local/state/self-heal.log`.

| Tier | Trigger | Action |
| ---- | ------- | ------ |
| 0 | Any failed verification | Restart the indexer, re-enable `sir.control` indexing, requeue pending changes that exhausted their retries |
| 1 | Replication older than the staleness window | Pause the indexer (frees disk I/O), run a replication catch-up to the upstream boundary, re-enable indexing |
| 2 | 3 consecutive failed verifications (~1h of persistent failure), at most once per day | Rebuild Solr from the verified local backup cache: stop `indexer` + `search`, wipe the Solr data directory, run `./setup-musicbrainz` |

Tier 2 deserves explanation: it wipes coordination state, not source data.
The MusicBrainz databases and the verified 66GB backup cache on the data disk
are untouched; the rebuild re-restores every collection from the local cache.
During the rebuild (a few hours on slow disks) the website's database-driven
pages keep working — only `/ws/2` search is unavailable.

Healing is automatic and idempotent. Set `MUSICBRAINZ_AUTO_HEAL=0` in the
environment of the service to disable all remedies (the timer still verifies
and logs). To run the check manually: `./scripts/self-heal.sh`.
